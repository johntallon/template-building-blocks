...
let collectionValidations = {
    ...
    throughput: (value, parent) => {
        ...
        if (_.isSafeInteger(value) && ((value % 100) !== 0)) {
            return {
                result: false,
                message: 'Value must be an integer and a multiple of 100'
            };
        }

        if (v.utilities.isNullOrWhitespace(parent.partitionKeyPath)) {
            return {
...
let cosmosDbValidations = {
        ...
        return {
            validations: {
                ...
                maxStalenessPrefix: (value, parent) => {
                    ...
                    return {
                        result: _.inRange(value, 1, 2147483648),
    ...
    ipRangeFilters: (value) => {
        ...
        return {
            validations: (value) => {
                return {
                    result: v.utilities.networking.isValidIpAddress(value) || v.utilities.networking.isValidCidr(value),
                    message: 'Value must be an IP address or an IP address range in CIDR format'
                };
            }
        };
    },
...
function transform(settings) {
    ...
    if (settings.ipRangeFilters.length > 0) {
        result.properties.ipRangeFilter = settings.ipRangeFilters.join(',');
    }
